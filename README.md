...
Berdasarkan hasil pengujian implementasi arsitektur kami dapat terdapat beberapa kesimpulan yang dapat diambil. Pertama, penggunaan tipe algoritma load balancing cukup berpengaruh pada hasil pengetesan dimana algoritma least connection memiliki performa yang lebih bagus dibandingkan algoritma round robin. Hal tersebut dapat dilihat dari jumlah peak concurrency algoritma least connection yang lebih tinggi pada setiap spawn rate yang diujikan. Kedua, jika algoritma yang digunakan adalah least connection maka jumlah peak concurrency ataupun spawn rate dapat diprediksi dengan melihat salah satunya, apabila spawn rate meningkat sebanyak 4 kali dari nilai awal, maka peak concurrency akan berkurang sekitar 2.7%. jika algoritma yang digunakan adalah round robin maka jumlah peak concurrency ataupun spawn rate dapat diprediksi dengan melihat salah satunya, apabila spawn rate meningkat sebanyak 1 kali dari nilai awal, maka peak concurrency akan berkurang sekitar 3% - 5%. Ketiga, pada beberapa tipe pengujian dapat dilihat bahwa nilai respon time meningkat secara tiba - tiba pada pertengahan testing dan menurun dengan sendirinya, hal tersebut mungkin mengindikasikan bahwa terdapat suatu pembatasan penggunaan CPU pada implementasi database kami. Oleh karena itu, disarankan untuk mengoptimalkan kemampuan database, khususnya dalam menangani permintaan GET dalam skenario penggunaan yang ekstrem, guna memastikan kinerja yang responsif dan konsisten di bawah tekanan beban yang tinggi.
